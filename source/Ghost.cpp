...
void Ghost::reset(std::pair<int, int> location, E_Direction direction)
{
	...
	this->stepped_on = Map::getPixelByName("Space");
...
void Ghost::make_move()
{
	...
	if (stuff_in_front == Map::getPixelByName("Lesser food") ||
		stuff_in_front == Map::getPixelByName("Greater food") ||
		stuff_in_front == Map::getPixelByName("Space") ||
		stuff_in_front == Map::getPixelByName("Ghost"))
	{
		this->instance->set_pixel(this->location, this->stepped_on);
